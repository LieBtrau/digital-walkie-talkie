...
def callback(in_data, frame_count, time_info, status):
    ...
    """
    data1 = wf.readframes(int(chunk * wf.getframerate() / AUDIO_SAMPLE_RATE))
    data3 = np.frombuffer(data1, dtype=np.int16)
    snd_data = np.asarray(samplerate.resample(data3, AUDIO_SAMPLE_RATE / wf.getframerate(), 0), dtype=np.int16)
