...
public:
	typedef struct
	{
		char name[RADIOLIB_AX25_MAX_CALLSIGN_LEN + 1] = {0}; //!<<6 bytes name
		byte ssid = 0;										 //!<<4 bit ssid
	} CallSign;
	typedef enum
	{
		DESTINATION,
	...
	/*!
	  ...
	  */
	AX25Frame(const char *destCallsign, byte destSSID, const char *srcCallsign, byte srcSSID, byte control);
	...
	/*!
	  ...
	  */
	AX25Frame(const char *destCallsign, byte destSSID, const char *srcCallsign, byte srcSSID, byte control, byte protocolID, const char *info);
	...
	/*!
	  ...
	  */
	AX25Frame(const char *destCallsign, byte destSSID, const char *srcCallsign, byte srcSSID, byte control, byte protocolID, byte *info, uint16_t infoLen);
