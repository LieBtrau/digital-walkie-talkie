...
static int handle_packet_rx(struct si446x_device *dev)
{
    ...
    { // Error but no callback registered
        return err;
    }
...
int si446x_reinit(struct si446x_device *dev)
{
    ...
    err = config_interrupts(dev, PH_INT_STATUS_EN, PACKET_RX | PACKET_SENT | CRC_ERROR | TX_FIFO_ALMOST_EMPTY | RX_FIFO_ALMOST_FULL, 0, 0);
    if (err)
    {
        return error(-err, __FILE__, __LINE__);
    }
